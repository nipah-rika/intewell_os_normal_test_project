...
此例程演示TTOS的在定时器中断里面进行AD7616采样的控制方法,156us采样一次,在定时器中断里面触发ADC转换,在BUSY下降沿中断进行SPI接口通讯读取数据,演示读取芯片内的ADC测试通道数据输出,当通道选择测试通道时，AD7616会一直输出0XAA(固定值,芯片数据手册上有)。
此例程只有突发模式调通了。
此例程经过了优化,配有ADC性能测试图片,读取16通道ADC采样数据只需13us。
