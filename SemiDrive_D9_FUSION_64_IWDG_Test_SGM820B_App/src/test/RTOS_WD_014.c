...
【测试目的】:
单TTOS多核下绑定核，正常使用看门狗的情况，启动看门狗+超时喂狗+停止看门狗，查看TTOS运行是否正常【不会重启也不会停止】。
...
T_MODULE T_VOID create_wd_task_cb (T_VOID *p_arg)
{
	...
	vmk_rc = TTOS_StartWatchdog(WD_TIMEOUT, FALSE);
	if (VMK_OK != vmk_rc)
	{
...
T_MODULE T_VOID  feed_wd_task_cb (T_VOID *p_arg)
{
	...
	T_VMK_ReturnCode vmk_rc;
	/*不及时喂狗*/
    rc = TTOS_SleepTask(2*WD_TIMEOUT);
	...
	/*因为不及时喂狗，即使过时再喂狗不能改变既定事实，喂狗应该失败*/
