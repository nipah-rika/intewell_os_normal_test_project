...
if [ "$dir" != "$dest_dir" ]
then
    echo "$dest_dir *(rw,sync,no_subtree_check,sec=none,no_root_squash,nohide)" >> /etc/exports
    exportfs -r
fi
...
do
    ...
    then
        echo "$dest_dir/$vm_dir *(rw,sync,no_subtree_check,sec=none,no_root_squash,nohide)" >> /etc/exports
        exportfs -r
    fi
