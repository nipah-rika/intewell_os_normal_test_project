...
$RTENV/hacl load $DOWNLOAD/vm1Config.bin -a 0x4f914000
if [ $? -ne 0 ]
then
    echo "$RTENV/hacl load $DOWNLOAD/vm1Config.bin -a 0x4f914000 fail"
    exit 1
fi

$RTENV/hacl load $DOWNLOAD/vm2Config.bin -a 0x4f915000
if [ $? -ne 0 ]
then
    echo "$RTENV/hacl load $DOWNLOAD/vm2Config.bin -a 0x4f915000 fail"
    exit 1
fi
