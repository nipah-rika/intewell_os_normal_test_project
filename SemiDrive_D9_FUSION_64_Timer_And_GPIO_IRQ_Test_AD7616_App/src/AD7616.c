...
//	 _set_sample_compeleted_irq_enable(info);//转换完成中断使能
}
	...
	ad7616_info_t *info = (ad7616_info_t *)arg;
	 ...
	 if(_Config & AD7616_SEQEN)//配置了序列化模式
	 {
			 if(_Config & AD7616_BURSTEN) //突发模式
			 {
			   ...
			   {
				    _read_squence_channel(info,&channel_a_adc,&channel_b_adc);//读取顺序通道号
				    TASK_LOG(" step1 next_channel_index:%d\r\n",info->next_channel_index);
			    	...
			    	{
					    TASK_LOG("\t\t step2 next_channel_index:%d\r\n",info->next_channel_index);
			    		 _start_convert(info);//启动转换
			    	}
				}
	 }
...
int userAppInit(void)
{
	...
	TASK_LOG("\t\tTTOS_GetSysClkRate1:%ld\r\n",sysClkRate );
	T_UDWORD freqCPU;
		VMK_GetCPUFreq(&freqCPU);
		printk("freqCPU = %llu, \n",freqCPU);
	TASK_LOG("\t\freqCPU:%ld\r\n",freqCPU);
	TASK_LOG("\t\tTTOS_GetSysClkRate:%d\r\n",TTOS_GetSysClkRate());
	AD7616_Test();
}
